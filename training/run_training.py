...
if __name__ == "__main__":
    ...
    ckpt_path  = os.path.join(CFG.export_dir, 'litevoicenet_best.pt')

    archive_dir = os.path.join(CFG.export_dir, 'archive')
    os.makedirs(archive_dir, exist_ok=True)
    import shutil, datetime
    ts = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
    for fname in ['litevoicenet_best.pt', 'litevoicenet.onnx', 'litevoicenet_int8.onnx']:
        src = os.path.join(CFG.export_dir, fname)
        if os.path.exists(src):
            shutil.copy2(src, os.path.join(archive_dir, '{}_{}'.format(ts, fname)))
    print("Archived previous models to {}/{}*".format(archive_dir, ts))
